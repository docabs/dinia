...
import { createDinia, defineStore, setActiveDinia, skipHydrate } from '../src'
import { computed, nextTick, reactive, ref, watch, customRef } from 'docuejs'
...
describe('State', () => {
  ...
  it('can reset the state of an empty store', () => {
    const store = defineStore('a', {})(createDinia())
    expect(store.$state).toEqual({})
    store.$reset()
    expect(store.$state).toEqual({})
  })
