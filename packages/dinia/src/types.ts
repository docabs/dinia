...
> = _StoreWithState<Id, S, G, A> &
  ...
  DiniaCustomProperties<Id, S, G, A> &
  DiniaCustomStateProperties<S>
 ...
 */
export interface DiniaCustomProperties<
  Id extends string = string,
  S extends StateTree = StateTree,
  G /* extends GettersTree<S> */ = _GettersTree<S>,
  A /* extends ActionsTree */ = _ActionsTree
...
/**
 * Properties that are added to every `store.$state` by `dinia.use()`.
 */
export interface DiniaCustomStateProperties<S extends StateTree = StateTree> {}
...
export type _GettersTree<S extends StateTree> = Record<
  string,
  | ((state: UnwrapRef<S> & UnwrapRef<DiniaCustomStateProperties<S>>) => any)
  | (() => any)
>
