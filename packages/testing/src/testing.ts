...
import type { ComputedRef, WritableComputedRef } from 'docuejs'
import {
  Dinia,
  DiniaPlugin,
  setActiveDinia,
...
export interface TestingOptions {
  ...
  /**
   * Creates an empty App and calls `app.use(dinia)` with the created testing
   * dinia. This allows you to use plugins while unit testing stores as
   * plugins **will wait for dinia to be installed in order to be executed**.
   * Defaults to false.
   */
  fakeApp?: boolean
...
/**
 * Dinia instance specifically designed for testing. Extends a regular
 * `Dinia` instance with test specific properties.
 */
export interface TestingDinia extends Dinia {
  /** App used by Dinia */
  app: App
}
...
/**
 * Creates a dinia instance designed for unit tests that **requires mocking**
 ...
 * `createSpy` option.
 *
 * @param options - options to configure the testing dinia
 * @returns a augmented dinia instance
 */
export function createTestingDinia({
  initialState = {},
  plugins = [],
  stubActions = true,
  stubPatch = false,
  ...
  plugins.forEach((plugin) => dinia._p.push(plugin))

  // allow computed to be manually overridden
  dinia._p.push(WritableComputed)
  ...
  if (!createSpy) {
    throw new Error(
      '[@dinia/testing]: You must configure the `createSpy` option.'
    )
  }

  // stub actions
  dinia._p.push(({ store, options }) => {
  ...
  Object.defineProperty(dinia, 'app', {
    configurable: true,
    enumerable: true,
    get(): App {
      return this._a
...
function WritableComputed({ store }: DiniaPluginContext) {
